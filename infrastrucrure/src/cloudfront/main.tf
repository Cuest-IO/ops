...
resource "aws_cloudfront_distribution" "web_bucket_distribution" {
  origin {
    domain_name = var.web_bucket_website_domain
    origin_id   = "${var.environment}-WebBucketS3Origin"
  ...
  enabled             = true
  is_ipv6_enabled     = true
  comment             = "${var.comment_prefix} ${var.web_bucket_website_domain}"
  default_root_object = "index.html"
  aliases = ["${var.environment}.web.${var.domain_name}", "www.${var.environment}.web.${var.domain_name}"]

  default_cache_behavior {
    allowed_methods  = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "${var.environment}-WebBucketS3Origin"
 ...
 viewer_certificate {
  ...
  depends_on = [var.web_certificate_validation]
}


resource "aws_cloudfront_origin_access_identity" "web_oai" {
  comment = "OAI for ${var.web_bucket_website_domain}"
}
